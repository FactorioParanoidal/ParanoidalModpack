...
local function MakeBatteryRecipe(name)
	...
	local ingr = {
		{"battery", math.ceil(get_fuel_value(name) / battery_capacity)},
		{"steel-plate", 2*level},
		ingr_advc,
	}
...
local charging_power = util.parse_energy(data.raw["furnace"][name_chg1].energy_usage)
...
local function MakeChargingRecipe(name)
	...
	return {
		...
		enabled = false,
		energy_required = math.ceil(get_fuel_value(name) / (charging_power*60)),
