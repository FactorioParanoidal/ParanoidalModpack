...
local Direction = require('__Kux-CoreLib__/stdlib/area/direction') --[[@as StdLib.Area.Direction]]
local Orientation = require('__Kux-CoreLib__/stdlib/area/orientation') --[[@as StdLib.Area.Orientation]]
...
function Position.from_chunk_position(pos)
    local x, y = (floor(pos.x or pos[1]) * 32), (floor(pos.y or pos[2]) * 32)
    return new(x, y)
end
...
local function load_area(area)
    local Area = package.loaded[AREA_PATH..".lua"]
    if not Area then
