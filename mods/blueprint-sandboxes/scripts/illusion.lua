...
Illusion.mappings = {
	...
	-- space exploration not used in this modpack!
}
...
function Illusion.HandleBlueprintEvent(player, potentialItemStacks)
	...
	if not entities then
		...
		if game.tick - lastWarningForNewContents > 216000 then -- 1 hour
			player.print(
				"WARNING: Known issues in Factorio prevent mods from seeing or updating Blueprints when using 'Select new contents'."
			)
			player.print(
				"This mod requires that ability to swap the Fake Illusions for their Real Entities in your Blueprints."
			)
			player.print(
				"If you are including any Fake Illusions in this Blueprint, they likely will NOT be replaced, especially if the source Blueprint is within the Library instead of your Inventory."
			)
