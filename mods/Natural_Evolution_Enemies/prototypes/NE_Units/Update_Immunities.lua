...
for type_name, type_prot in pairs(data.raw) do
    for element_name, element_prot in pairs(type_prot) do
        if element_prot.minable or element_prot.collision_box or element_prot.selection_box then
        ...
        if element_prot.name and element_prot.type == "item" or element_prot.stack_size or element_prot.place_result or
            element_prot.place_as_tile then
        ...
        elseif ent_type.resistances then
            ...
            if fire then
                if nefire then
                    ...
                    if ent_type.type == "unit-spawner" then
                        table.insert(ent_type.resistances, {
                            type = 'ne_fire',
                            percent = 100
                        }) -- add new nefire
                    else
...
NE_Functions.Add_ALL_Damage_Resists_to_Unit_type(data.raw["unit"]["ne-biter-megalodon"],
    24 + NE_Enemies.Settings.NE_Difficulty) -- 25% resistance to everything
...
for k, v in pairs(data.raw["damage-type"]) do
    local Damage_type = {
        type = "damage",
        damage = {
            amount = (20 + (NE_Enemies.Settings.NE_Difficulty * 5)),
            type = k
        }
    }
    table.insert(data.raw.unit["ne-biter-megalodon"].attack_parameters.ammo_type.action.action_delivery.target_effects,
        Damage_type)
end
