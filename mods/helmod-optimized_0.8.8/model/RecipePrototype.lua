...
function RecipePrototype.load(object, object_type)
  ...
  -- if recipe_load == false then
  --   recipe_load = true
  --   recipes = game.forces.player.recipes
  -- end
  ...
  if lua_type == nil or lua_type == "recipe" then
    ...
    -- lua_prototype = recipes[object_name]
    lua_type = "recipe"
...
function RecipePrototype.isVoid()
  if is_voider == nil then
    RecipePrototype.getProducts()
  end
  return is_voider
end
