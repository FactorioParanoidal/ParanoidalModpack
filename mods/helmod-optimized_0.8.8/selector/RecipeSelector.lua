...
local RecipePrototypes = require "model.RecipePrototypes"
...
-- @return boolean
--
function RecipeSelector.methods:checkFilter(name, type)
  ...
  if filter_prototype ~= nil and filter_prototype ~= "" then
    local elements = RecipePrototypes.getProducts(name, type)
    if filter_prototype_product ~= true then
      elements = RecipePrototypes.getIngredients(name, type)
    end
...
function RecipeSelector.methods:appendGroups(name, type, list_group, list_subgroup, list_prototype)
  if type == "recipe" or type == "resource" then
    RecipePrototypes.load(name, type)
    local find = self:checkFilter(name, type)
    ...
    if
      find == true and (RecipePrototypes.getEnabled(name, type) == true or filter_show_disable == true) and
        (RecipePrototypes.getHidden(name, type) == false or filter_show_hidden == true)
     then
      local lua_recipe = RecipePrototypes.native(name, type)
      local group_name = lua_recipe.group.name
      local subgroup_name = lua_recipe.subgroup.name

      if firstGroup == nil then
        firstGroup = group_name
      end
      list_group[group_name] = lua_recipe.group
      list_subgroup[subgroup_name] = lua_recipe.subgroup
      if list_prototype[group_name] == nil then
        list_prototype[group_name] = {}
      end
      if list_prototype[group_name][subgroup_name] == nil then
        list_prototype[group_name][subgroup_name] = {}
      end
...
function RecipeSelector.methods:buildPrototypeIcon(guiElement, prototype, tooltip)
  Logging:trace(
    self:classname(),
    "buildPrototypeIcon(player, guiElement, prototype, tooltip:",
    guiElement,
    prototype,
    tooltip
  )
  ...
  ElementGui.addGuiButtonSelectSprite(
    guiElement,
    self:classname() .. "=element-select=ID=" .. type .. "=",
    Player.getRecipeIconType(RecipePrototype.native()),
    prototype_name,
    prototype_localised_name,
    tooltip,
    color
  )
end
