...
--log("Creating Types")
if not heroturrets.defines then
...
for category, prototypes in pairs(data.raw) do
	...
	if name and prototype.stack_size then
		table.insert(types_with_items, category)
	end
end
--end honk
...
local is_nesw = function(entity)
	...
	if entity.base_picture == nil then
		return false
	end
	if entity.base_picture.north == nil then
		return false
	end
	if entity.base_picture.north.layers == nil then
		return false
	end
	if entity.base_picture.east == nil then
		return false
	end
	if entity.base_picture.east.layers == nil then
		return false
	end
	if entity.base_picture.south == nil then
		return false
	end
	if entity.base_picture.south.layers == nil then
		return false
	end
	if entity.base_picture.west == nil then
		return false
	end
	if entity.base_picture.west.layers == nil then
		return false
	end
	return true
end
...
local is_unkown_nesw = function(entity)
	...
	if box == nil then
		box = entity.collision_box
	end

	if
		(entity.type == "ammo-turret" or entity.type == "electric-turret")
		and entity.base_picture == nil
		and box ~= nil
	then
		return true
	end
	...
	for _, tech in pairs(tech) do
		if tech.name:match("^refined%-flammables%-") and tech.effects then
			...
			if effect ~= nil then
				...
				table.insert(
					tech.effects,
					{ type = "turret-attack", turret_id = name, modifier = effect.modifier * (1 + (0.25 * rank)) }
				)
			else
				error("Missing tech for " .. tech.name .. " for " .. entity.name)
			end
		end
	end
end
...
local local_create_turret = function(turret, rank, rank_string, mod)
	...
	if
		settings.startup["heroturrets-allow-artillery-turrets"].value == false
		and turret.entity.type == "artillery-turret"
	then
	...
	if entity.rotation_speed ~= nil then
		entity.rotation_speed = entity.rotation_speed * mod
	end
	--if entity.turret_rotation_speed ~= nil then entity.turret_rotation_speed = entity.turret_rotation_speed*mod end
	if entity.preparing_speed ~= nil then
		entity.preparing_speed = entity.preparing_speed * mod
	end
	if entity.attacking_speed ~= nil then
		entity.attacking_speed = entity.attacking_speed * mod
	end
	if entity.prepare_range ~= nil then
		entity.prepare_range = entity.prepare_range * mod
	end
	if entity.turret_rotation_speed ~= nil then
		entity.turret_rotation_speed = entity.turret_rotation_speed * mod
	end
	if entity.manual_range_modifier ~= nil then
		entity.manual_range_modifier = entity.manual_range_modifier * mod
	end
	...
	if entity.attack_parameters ~= nil then
		if entity.attack_parameters.cooldown ~= nil then
			entity.attack_parameters.cooldown = entity.attack_parameters.cooldown / mod
		end
		if entity.attack_parameters.range ~= nil then
			entity.attack_parameters.range = entity.attack_parameters.range * mod
		end
		...
		if entity.attack_parameters.ammo_type ~= nil and entity.attack_parameters.ammo_type.action ~= nil then
			if
				entity.attack_parameters.ammo_type.action.action_delivery ~= nil
				and entity.attack_parameters.ammo_type.action.action_delivery.max_length ~= nil
			then
				entity.attack_parameters.ammo_type.action.action_delivery.max_length = entity.attack_parameters.ammo_type.action.action_delivery.max_length
					* mod
			...
			if
				entity.attack_parameters.ammo_type.action.action_delivery ~= nil
				and entity.attack_parameters.ammo_type.action.action_delivery.max_range ~= nil
			then
				entity.attack_parameters.ammo_type.action.action_delivery.max_range = entity.attack_parameters.ammo_type.action.action_delivery.max_range
					* mod
	...
	if entity.gun ~= nil then
		...
		if gun.attack_parameters ~= nil then
			...
			if gun.attack_parameters.cooldown ~= nil then
				gun.attack_parameters.cooldown = gun.attack_parameters.cooldown / mod
			end
			if gun.attack_parameters.range ~= nil then
				gun.attack_parameters.range = gun.attack_parameters.range * mod
			end
			gun.name = gun_name
	...
	if is_nesw(entity) then
		if left == nil then
			left = entity.base_picture.north.layers[1].width / 2
		end
		if top == nil then
			top = entity.base_picture.north.layers[1].height / 2
		end
		table.insert(
			entity.base_picture.north.layers,
			get_badge(rank, top, left, entity.base_picture.north.layers[1].repeat_count or 1)
		)

		if left == nil then
			left = entity.base_picture.east.layers[1].width / 2
		end
		if top == nil then
			top = entity.base_picture.east.layers[1].height / 2
		end
		table.insert(
			entity.base_picture.east.layers,
			get_badge(rank, top, left, entity.base_picture.north.layers[1].repeat_count or 1)
		)

		if left == nil then
			left = entity.base_picture.south.layers[1].width / 2
		end
		if top == nil then
			top = entity.base_picture.south.layers[1].height / 2
		end
		table.insert(
			entity.base_picture.south.layers,
			get_badge(rank, top, left, entity.base_picture.north.layers[1].repeat_count or 1)
		)

		if left == nil then
			left = entity.base_picture.west.layers[1].width / 2
		end
		if top == nil then
			top = entity.base_picture.west.layers[1].height / 2
		end
		table.insert(
			entity.base_picture.west.layers,
			get_badge(rank, top, left, entity.base_picture.north.layers[1].repeat_count or 1)
		)
	elseif is_unkown_nesw(entity) and left ~= nil and top ~= nil then
		...
		if left == nil then
			left = entity.base_picture.layers[1].width / 2
		end
		if left == nil then
			top = entity.base_picture.layers[1].height / 2
		end

		table.insert(
			entity.base_picture.layers,
			get_badge(rank, top, left, entity.base_picture.layers[1].repeat_count or 1)
		)
	end
	...
	if turret.entity.fast_replaceable_group == nil then
		...
		then
			...
			if turret.entity.next_upgrade == nil then
				turret.entity.next_upgrade = entity_name
			end
		end
	else
		if
	...
	if entity.type == "ammo-turret" then
		update_ammo_turret_tech(turret.entity, entity_name, rank)
	end
	if entity.type == "fluid-turret" then
		update_fluid_turret_tech(turret.entity, entity_name, rank)
	end
...
local local_get_names = function()
	...
	if custom_string == nil then
		return names
	end
	custom_string = local_trim(custom_string)

	if #custom_string == 0 then
		return names
	end
...
local local_create_turrets = function() --personal-laser-defense-equipment  "active-defense-equipment"
	...
	local guns = {}
	for at = 1, #turret_types do
		local tt = turret_types[at]
		for name, entity in pairs(data.raw[tt]) do
			...
			if
				entity ~= nil
				and entity.name ~= nil
				and entity.type ~= "active-defense-equipment"
				...
				and (
					is_nesw(entity)
					or (entity.base_picture ~= nil and entity.base_picture.layers ~= nil)
					or (is_unkown_nesw(entity) and left ~= nil and top ~= nil)
				) -- or (entity.cannon_base_pictures ~= nil and entity.cannon_base_pictures.layers ~= nil and entity.cannon_base_pictures.layers[1].direction_count == 256))
				and entity.max_health > 1
				and entity.minable ~= nil
				and entity.minable.result ~= nil
			then
				local recipe = local_get_recipe(entity.name)
				if recipe == nil then
					log("Missing recipie for " .. " name " .. name .. " type " .. tt)
				end
				local item = local_get_item(entity.name)
				if item == nil then
					log("Missing item for " .. " name " .. name .. " type " .. tt)
				end
				if item ~= nil and entity.minable.result ~= item.name then
					log(
						"Mining result mismatch entity.minable.result "
				...
				if recipe == nil and item ~= nil then
					recipe = data.raw["recipe"][item.name]
				end
...
local local_update_ammo_ranges = function()
	for k, _ in pairs(data.raw["ammo"]) do
		if
			data.raw["ammo"][k].ammo_type ~= nil
			and data.raw["ammo"][k].ammo_type.category ~= nil
			and cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category] ~= nil
		then
				...
				if #data.raw["ammo"][k].ammo_type.action > 0 then
					for j = 1, #data.raw["ammo"][k].ammo_type.action do
						if data.raw["ammo"][k].ammo_type.action[j].action_delivery ~= nil then
							if
								data.raw["ammo"][k].ammo_type.action[j].action_delivery.max_range ~= nil
								and data.raw["ammo"][k].ammo_type.action[j].action_delivery.max_range
									< cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category]
							then
								data.raw["ammo"][k].ammo_type.action[j].action_delivery.max_range =
									cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category]
								log(
									"updated "
										.. data.raw["ammo"][k].name
										.. " range to "
										.. cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category]
								)
							end
					...
					if data.raw["ammo"][k].ammo_type.action.action_delivery ~= nil then
						if
							data.raw["ammo"][k].ammo_type.action.action_delivery.max_range ~= nil
							and data.raw["ammo"][k].ammo_type.action.action_delivery.max_range
								< cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category]
						then
							data.raw["ammo"][k].ammo_type.action.action_delivery.max_range =
								cannon_ammo_ranges[data.raw["ammo"][k].ammo_type.category]
							log(
								"updated "
									.. data.raw["ammo"][k].name
