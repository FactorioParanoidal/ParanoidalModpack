...
function post_process.index_properties()
	...
	local function index_icons()
		...
		if not OSM.log.missing_icons then
			OSM.log.missing_icons = {}
		end
		...
		local function index_icon(prototype, external_prototype)
			...
			if prototype.icon_size or (external_prototype and external_prototype.icon_size) then
				icon_size = prototype.icon_size or external_prototype.icon_size
			end
			if prototype.icon_mipmaps or (external_prototype and external_prototype.icon_mipmaps) then
				icon_mipmaps = prototype.icon_mipmaps or external_prototype.icon_mipmaps
			end
			if prototype.scale or (external_prototype and external_prototype.scale) then
				scale = prototype.scale or external_prototype.scale
			end
			if prototype.tint or (external_prototype and external_prototype.tint) then
				tint = prototype.tint or external_prototype.tint
			end
			if prototype.shift or (external_prototype and external_prototype.shift) then
				shift = prototype.shift or external_prototype.shift
			end
			...
			if icons_index[1] then
				if not OSM.table.prototype_index[prototype.type] then
					OSM.table.prototype_index[prototype.type] = {}
				end
				if not OSM.table.prototype_index[prototype.type][prototype.name] then
					OSM.table.prototype_index[prototype.type][prototype.name] = {}
				end
			...
			elseif OSM.table.prototype_index[prototype.type] and not no_icon_index[prototype.type] then
				table.insert(
					OSM.log.missing_icons,
					"Info: Could not index icon for: (" .. prototype.type .. ") " .. '"' .. prototype.name .. '"'
				)
			end
		end
	...
	local function index_subgroups()
		if not OSM.log.missing_subgroup then
			OSM.log.missing_subgroup = {}
		end
		...
		local function index_subgroup(prototype, external_prototype)
			...
			if external_prototype then
				if external_prototype.subgroup then
					subgroup = external_prototype.subgroup
				end
				if external_prototype.order then
					order = external_prototype.order
				end
			end

			if subgroup then
				OSM.table.prototype_index[prototype.type][prototype.name].subgroup = subgroup
			end
			if order then
				OSM.table.prototype_index[prototype.type][prototype.name].order = order
			end

			if not OSM.table.prototype_index[prototype.type][prototype.name].subgroup then
				table.insert(
					OSM.log.missing_subgroup,
					"Could not index subgroup for: (" .. prototype.type .. ") " .. '"' .. prototype.name .. '"'
				)
			end
		end
		...
		for item_type, _ in pairs(OSM.data.item) do
			if data.raw[item_type][prototype_name] or data.raw.fluid[prototype_name] then
				...
				if not prototype.subgroup then
					table.insert(
						OSM.log.missing_subgroup,
						prototype_type .. '"' .. prototype.name .. '"' .. " does not have a subgroup"
					)
				end
			end
		end
	...
	local function index_unlocks()
		for _, technology in pairs(data.raw.technology) do
			if technology.effects then
				if not OSM.table.prototype_index[technology.type] then
					OSM.table.prototype_index[technology.type] = {}
				end
				if not OSM.table.prototype_index[technology.type][technology.name] then
					OSM.table.prototype_index[technology.type][technology.name] = {}
				end
	...
	local function index_autoplaces()
		for _, resource in pairs(data.raw.resource) do
			if resource.autoplace and resource.autoplace.control then
				if not OSM.table.prototype_index["autoplace-control"] then
					OSM.table.prototype_index["autoplace-control"] = {}
				end
				if not OSM.table.prototype_index["autoplace-control"][resource.autoplace.control] then
					OSM.table.prototype_index["autoplace-control"][resource.autoplace.control] = {}
				end
				if not OSM.table.prototype_index["autoplace-control"][resource.autoplace.control].resources then
					OSM.table.prototype_index["autoplace-control"][resource.autoplace.control].resources = {}
				end
...
function post_process.get_additional_prototypes()
	...
	if settings.startup["deadlock-stack-size"] then
		for _, item in pairs(OSM.table.disabled_prototypes["item"]) do
			if
				data.raw[item.type]["deadlock-stack-" .. item.name]
				and not OSM.table.enabled_prototypes["item"][item.name]
			then
				OSM.table.disabled_prototypes["item"]["deadlock-stack-" .. item.name] = {}
				OSM.table.disabled_prototypes["item"]["deadlock-stack-" .. item.name].name = "deadlock-stack-"
					.. item.name
				...
				OSM.table.disabled_prototypes["recipe"]["deadlock-stacks-stack-" .. item.name].name = "deadlock-stacks-stack-"
					.. item.name
				...
				OSM.table.disabled_prototypes["recipe"]["deadlock-stacks-unstack-" .. item.name].name = "deadlock-stacks-unstack-"
					.. item.name
				OSM.table.disabled_prototypes["recipe"]["deadlock-stacks-unstack-" .. item.name].mod_name =
					item.mod_name
			end
		end
		...
		if mods["deadlock_stacked_recipes"] then
			for _, recipe in pairs(OSM.table.disabled_prototypes["recipe"]) do
				if
					data.raw.recipe["StackedRecipe-" .. recipe.name]
					and not OSM.table.enabled_prototypes["recipe"][recipe.name]
				then
					OSM.table.disabled_prototypes["recipe"]["StackedRecipe-" .. recipe.name] = {}
					OSM.table.disabled_prototypes["recipe"]["StackedRecipe-" .. recipe.name].name = "StackedRecipe-"
						.. recipe.name
...
function post_process.disable_prototypes()
	...
	local function disable_technology(technology)
		if not data.raw.technology[technology.name] then
			return
		end
					...
					if prerequisite == technology.name then
						...
						table.insert(
							...
							"info: "
								...
								.. tech.name
								.. '"'
								.. " prerequisites because it was disabled by: "
								.. '"'
								.. mod_name
								.. '"'
						)
		...
		if not OSM.log.disabled_prototypes[mod_name] then
			OSM.log.disabled_prototypes[mod_name] = {}
		end
		table.insert(
			OSM.log.disabled_prototypes[mod_name],
			"Info: " .. '"' .. mod_name .. '"' .. ": disabled technology: " .. '"' .. technology.name .. '"'
		)
	end
	...
	local function disable_recipe(recipe)
		if data.raw.recipe[recipe.name] then
					...
					for i, effect in pairs(technology.effects) do
						if effect.type == "unlock-recipe" and effect.recipe == recipe.name then
							...
							table.insert(
								...
								"Info: "
									.. '"OSM-Lib Post Process Script"'
									.. ": Removed unlock from technologies for: "
									.. '"'
									.. recipe.name
									.. '"'
									.. " because it was disabled by: "
									.. '"'
									.. mod_name
									.. '"'
							)
						...
						if limitation == recipe.name then
							...
							table.insert(
								...
								"Info: "
									...
									.. recipe.name
									.. '"'
									.. " from module limitation list because it was disabled by: "
									.. '"'
									.. mod_name
									.. '"'
							)
			...
			table.insert(
				OSM.log.disabled_prototypes[mod_name],
				"Info: " .. '"' .. mod_name .. '"' .. ": Disabled recipe: " .. '"' .. recipe.name .. '"'
			)
		end
	end
	...
	local function disable_fluid(fluid)
		if data.raw.fluid[fluid.name] then
			...
			table.insert(
				OSM.log.disabled_prototypes[mod_name],
				"Info: " .. '"' .. mod_name .. '"' .. ": Disabled fluid: " .. '"' .. fluid.name .. '"'
			)
		end
	end
		...
		for _, resource in pairs(data.raw.resource) do
			if string.find(resource.name, resource_name, 1, true) then
				...
				if resource.name ~= resource_name then
					table.insert(
						OSM.table.disabled_prototypes.resources,
						{ prototype_name = resource.name, mod_name = mod_name }
					)
				end
				...
				table.insert(
					OSM.log.disabled_prototypes[mod_name],
					"Info: " .. '"' .. mod_name .. '"' .. ": Disabled resource: " .. '"' .. resource_name .. '"'
				)
			end
		end
	end
	...
	local function enable_enlisted_prototypes()
		for _, prototype in pairs(OSM.table.enabled_prototypes["technology"]) do
			if OSM.table.disabled_prototypes["technology"][prototype.name] then
				...
				if not OSM.log.enabled_prototypes[mod_name] then
					OSM.log.enabled_prototypes[mod_name] = {}
				end
				table.insert(
					OSM.log.enabled_prototypes[mod_name],
					"Warning: "
						...
						.. mod_name
						.. '"'
						.. ": Prevents disabling of technology: "
						.. '"'
						.. prototype_name
						.. '"'
				)
			...
			if OSM.table.disabled_prototypes["resource"][prototype.name] then
				...
				table.insert(
					...
					"Warning: "
						...
						.. mod_name
						.. '"'
						.. ": Prevents disabling of resource: "
						.. '"'
						.. prototype_name
						.. '"'
				)
	...
	local function finalise_recipes()
		for _, recipe_prototype in pairs(data.raw.recipe) do
			if not recipe_prototype.OSM_removed then
				...
				if recipe_prototype.normal then
					table.insert(recipe_difficulty, recipe_prototype.normal)
				end
				if recipe_prototype.expensive then
					table.insert(recipe_difficulty, recipe_prototype.expensive)
				end
				...
				if recipe_prototype.subgroup == "other" or recipe_prototype.subgroup == "ee-tools" then
					goto skip
				end
				if
					recipe_prototype_result
					and (recipe_prototype_result.subgroup == "other" or recipe_prototype_result.subgroup == "ee-tools")
				then
					goto skip
				end
									...
									for _, technology in pairs(data.raw.technology) do
										if technology.effects then
											for _, effect in pairs(technology.effects) do
												if effect.recipe == recipe_prototype.name then
													soft_lock = false
												end
											end
										end
				...
				for _, resource in pairs(autoplace.resources) do
					...
					if resource and not resource.OSM_removed then
						...
						if has_disabled_resource then
							data.raw["autoplace-control"][i].localised_name =
								{ "", "[img=entity." .. resource.name .. "] ", { "entity-name." .. resource.name } }
						end
					end
				end
			end
			...
			if not keep_autoplace[resource.name] then
				...
				for _, preset in pairs(data.raw["map-gen-presets"]["default"]) do
					if
						...
						and preset.basic_settings.autoplace_controls[resource.name]
					then
...
function post_process.regenerate_properties()
	if not OSM.log.regenerated_subgroups then
		OSM.log.regenerated_subgroups = {}
	end
	if not OSM.log.regenerated_icons then
		OSM.log.regenerated_icons = {}
	end

	local function regenerate_properties(prototype)
		if not prototype.OSM_regenerate then
			return
		end
		if not OSM.table.prototype_index[prototype.type] then
			return
		...
		if OSM.table.prototype_index[prototype.type][prototype.name].icons then
			...
			table.insert(
				OSM.log.regenerated_icons,
				"Info: Regenerated icon for: (" .. prototype.type .. ") " .. '"' .. prototype.name .. '"'
			)
		end
		...
		if OSM.table.prototype_index[prototype.type][prototype.name].subgroup and not prototype.OSM_removed then
			...
			table.insert(
				OSM.log.regenerated_subgroups,
				"Regenerated subgroup for: (" .. prototype.type .. ") " .. '"' .. prototype.name .. '"'
			)
		end
		...
		if prototype.OSM_unlock_warning then
			table.insert(
				...
				"Warning: Recipe: "
					...
					.. prototype.name
					.. '"'
					.. " had the only technology unlocking it disabled by another mod"
			)
		end

		-- Add tooltips to faulty prototypes
		if
			prototype.OSM_soft_removed
			or prototype.OSM_result_warning
			or prototype.OSM_unlock_warning
			or prototype.OSM_ingredient_warning
		then
			prototype.localised_description =
				{ "", "[font=default-bold][color=#ffe6c0]Prototype type:[/color][/font] " .. prototype.type }
...
function post_process.debug_mode()
	...
	local function view_internal_names(prototype)
		if prototype.type == "map-gen-presets" or prototype.type == "gui-style" then
			return
		end -- Yeah, fuck you!
...
function post_process.print_log()
	...
	if data_length == dictionary_length then
		...
		for data_type, _ in pairs(data.raw) do
			if not OSM.data.raw[data_type] then
				log(data_type)
			end
		end
	end
