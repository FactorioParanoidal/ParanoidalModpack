...
local function research_basic_technologies_if_need()
	_table.each(get_player().force.technologies, function(technology)
		if not technology.enabled then
			return
		end
		if (not technology.research_unit_ingredients or _table.size(technology.research_unit_ingredients) == 0)
			and (settings.startup["hardcore-mode-for-playing-use-separated-technologies-for-every-resource"].value and not string.find(technology.name, DETECTED_RESOURCE_TECHNOLOGY_SUFFIX, 1, true)
				or not settings.startup["hardcore-mode-for-playing-use-separated-technologies-for-every-resource"].value) then
