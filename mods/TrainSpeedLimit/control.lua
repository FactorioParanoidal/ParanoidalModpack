...
function getLimitIndication(entity, red_network, green_network)
    local sign_value = entity.get_or_create_control_behavior().parameters[1].count
...
function setLimitIndication(entity, value)
    ...
    entity.get_or_create_control_behavior().parameters = {
        {index = 1, count = value, signal = {type="item", name="train-limit"}}
    }
	...
	if( checkThis ) then
		...
		if not valid then
			...
			if event.created_entity.name ~= "ghost-entity"  then 				
				game.players[event.player_index].insert( { name = item.name, count=20 } )
