...
function Log(what)
helpers.write_file("myrpg.log", serpent.block(what), true)
end
...
-- Flying-text entities were removed in 2.0; keep the text visible to all viewers.
local function flying_text(surface, position, text, color)
	for _, viewer in pairs(game.connected_players) do
		viewer.create_local_flying_text{surface=surface, position=position, text=text, color=color}
	end
end
...
if settings.get_player_settings(player)["charxpmod_print_xp_user"].value then
	...
	local surface = body and body.surface or player.surface
	local position = body and body.position or player.position
	flying_text(surface, position, "+" .. RPG_format_number(XP) ..' XP', colors.yellow)
	end
end
end
...
storage.setting_print_critical   =  false	
storage.setting_afk_time         =  15
storage.setting_time_ratio_xp    =  true
storage.setting_death_penal      =  30
...
storage.setting_allow_xp_by_tech   =  true	
storage.setting_allow_xp_by_kill   =  true	
storage.setting_allow_xp_by_rocket =  true
storage.setting_allow_xp_by_mining =  true
storage.XP_Mult                    =  1
storage.setting_allow_damage_attribs =  false	
storage.setting_max_player_level   =  200	
storage.setting_max_level_ability  =  50
...
storage.RPG_Bonus = {}
	for k=1,#storage.Player_Attributes do
		local attrib = storage.Player_Attributes[k]
		storage.RPG_Bonus[attrib]= mySettingsrpg ["charxpmod_"..attrib].value
...
-- GUI_Unifyer 2.0 no longer registers Beta 8's btcharxp button.
if script.active_mods["GUI_Unifyer"] then
	local flow = mod_gui.get_button_flow(player)
	if flow.btcharxp then flow.btcharxp.destroy() end
	flow.add{name="btcharxp", type="sprite-button", sprite="rpg_button",
		tooltip={"panel-title", player.name}, style=mod_gui.button_style}
end
...
function XP_Player_upd()
	...
	if name~='neutral' and name~='enemy' then
		...
		if cp>0 then
			local XP = storage.XP[name]   --math.ceil(global.XP[name] / cp)
...
--	if XP > 999999 then XP=999999 end
	XP = math.ceil((1+force.get_evolution_factor("nauvis")) * storage.XP_Mult * XP/3) 
...
if killer and killer.valid and storage.kills_units[force.name] and event.entity.force~=game.forces.neutral then
	if event.entity.prototype and event.entity.prototype.get_max_health() > 0 and (not force.get_friend(event.entity.force)) then
...
flying_text(entity.surface, entity.position, math.ceil(damage), colors.red)
end
end
	...
	if cause.type == 'character' and damage_type.name~='poison' and damage_type.name~='cold' then
		...
		if new_damage > original_damage_amount then 
			...
			if entity.health < dif then  -- give kill xp to player because the extra damage will kill entity
				...
				storage.last_overkill = event.entity
				end
...
if event.research.force then
	...
	if storage.XP_TECH[force] then
		...
		techXP = math.ceil(storage.XP_Mult * techXP * (1+ (6*game.forces["enemy"].get_evolution_factor("nauvis"))))		
...
local XP
	...
	for p, PL in pairs (force.connected_players) do 
		local r_count = storage.personalxp.rocketsXP_count[PL.name]
		if r_count == nil then r_count=0 end
		XP = math.ceil(storage.XP_Mult * storage.personalxp.XP[PL.name]/(5+(r_count*2))) --20% inicial
		storage.personalxp.XP[PL.name] = storage.personalxp.XP[PL.name] + XP
		printXP(PL,XP)
		storage.personalxp.rocketsXP_count[PL.name] = r_count+1
	end
end
end)
...
if ent.prototype.get_max_health() > 0 then
	XP=ent.prototype.get_max_health()
