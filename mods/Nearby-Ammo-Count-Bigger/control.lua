...
local draw_norm=	{target_offset={.0,-.1},only_in_alt_mode=true,scale=2.5,scale_with_zoom=true,alignment="center"}
local draw_low=		{target_offset={.0,-.1},only_in_alt_mode=true,scale=2.5,scale_with_zoom=true,alignment="center"}
local draw_crit=	{target_offset={.0,-.1},only_in_alt_mode=true,scale=2.5,scale_with_zoom=true,alignment="center"}
...
local function mag_size(m)
	...
	if x == nil then
		...
		x = prototypes.item[m].magazine_size
		_mag_size[m] = x
	end
	return x
end

--if prototypes.entity["infinity-chest"] then log("five") end

local function turret_mags(t)
	local x=_turret_mags[t]
	if x == nil then
		--x = entity_prototypes[t].automated_ammo_count
		x = prototypes.entity[t].automated_ammo_count
		_turret_mags[t] = x
	end
	return x
end
...
local drawparams={time_to_live=61,target_offset={.2,.375},forces={},only_in_alt_mode=true,scale=2.5,scale_with_zoom=true,alignment="right"}
