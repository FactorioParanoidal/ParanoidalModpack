...
local function hide_the_remote(recipe, technology, item)
	...
	if recipe_prototype then
		...
		if technology ~= nil and tech_prototype then
			...
			for i=1,(#effect) do
				if effect[i].type == "unlock-recipe" then -- SOMETHING IS WRONG: if data.raw["technology"]["artillery"].effects.type == "unlock-recipe" then -- ickputzdirwech
...
if settings.startup["artillery-jammer-remote"].value == true then
	hide_the_remote("artillery-jammer-tool", "artillery", data.raw["selection-tool"]["artillery-jammer-tool"])
end
-- ICKPUTZDIRWECH stop
...
-- ICKPUTZDIRWECH start
if settings.startup["vehicle-wagon-2-winch"].value == true then
...
if settings.startup["autogen"].value == true then
	...
	local shortcut_blacklist = {
		...
		"path-remote-control",
		"unit-remote-control",
		"fp_beacon_selector", -- ickputzdirwech
	}
