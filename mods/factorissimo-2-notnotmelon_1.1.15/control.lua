...
script.on_event({defines.events.on_built_entity, defines.events.on_robot_built_entity, defines.events.script_raised_built, defines.events.script_raised_revive}, function(event)
	...
	if has_layout(entity.name) then
		if event.robot and event.robot.name == 'companion-construction-robot' then
			error('\n\n\n\n[MOD INCOMPATIBILITY WITH COMPANION DRONES]\nYou cannot build a factory building with a companion drone. This error was caused because otherwise your save would be corrupted. There is a bug report on Klonan\'s mod page here: https://mods.factorio.com/mod/Companion_Drones/discussion/62e35931c6b3d5b8b627ada9.\nI cannot fix this from my end.\n\n\n\n')
		end
...
local fake_robots = {['companion-construction-robot'] = true, ['repair-block-robot'] = true} -- Modded construction robots with heavy control scripting
