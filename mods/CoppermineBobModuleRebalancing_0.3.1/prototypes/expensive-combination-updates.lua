if settings.startup["coppermine-bob-module-expensive-module-combination"].value then
  ...
  for product, recipe in pairs(data.raw.recipe) do
    ...
    if match_start then
      ...
      local level_s = product:sub(match_end + 2)
      local next_dash = level_s:find("-")
      if next_dash then
        level_s = level_s:sub(1, next_dash)
      end
      local level = tonumber(level_s)
      -- Detect other things with module in the name but more than just a level
      -- after (like the module requesters)
      local is_unrelated_recipe = level_s:len() > 2 and level == nil

      if is_unrelated_recipe then
        -- log("Unrelated recipe "..product.." (level "..level_s..")")
      else
