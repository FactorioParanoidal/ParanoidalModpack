...
-- Initialize AR's own remote-call interfaces
remote.add_interface("AbandonedRuins",
{
  ...
  enable_spawning_ruins = function(spawn_ruins)
    ...
    spawning.enable_spawning_ruins(spawn_ruins)
  ...
  ---@return boolean
  is_spawning_ruins_enabled = function() return spawning.is_spawning_ruins_enabled() end,

  -- Add ruin size and its halfed size
  ---@param size string Size as a lower-case word, e.g. "small"
  ---@param half_size number, Half of the ruin's size, e.g. 2
  ---@param multiplier number Multiplier for minimum distance to spawn point calculation e.g. 1
  register_ruin_size = function(size, half_size, multiplier)
    if debug_log then log(string.format("[register_ruin_size]: size[]='%s',half_size[]='%s',multiplier[]='%s' - CALLED!", type(size), type(half_size), type(multiplier))) end
    ...
    elseif half_size <= 0 then
      error(string.format("Parameter half_size=%d is smaller or equal zero", half_size))
    elseif type(multiplier) ~= "number" then
      error(string.format("Parameter multiplier[]='%s' is not expected type 'number'", type(multiplier)))
    elseif multiplier <= 0 then
      error(string.format("Parameter multiplier=%.2f is smaller or equal zero", multiplier))
    end

    if debug_log then log(string.format("[register_ruin_size]: Adding ruin size='%s',half_size=%d,multiplier=%.2f ...", size, half_size, multiplier)) end
    spawning.ruin_sizes[#spawning.ruin_sizes] = size
    spawning.ruin_half_sizes[size] = half_size
    spawning.ruin_min_distance_multiplier[size] = multiplier

    if debug_log then log("[register_ruin_size]: EXIT!") end
  end,
  ...
  -- The ruin-sets are not saved or loaded from save-game.
  ...
  spawn_exclusively_on = function(surface_name, ruinset_name)
    ...
    elseif #surface_name == 0 then
      error("Parameter 'surface_name' cannot be an empty string")
