...
function Moving_vehicle:request_path()
    ...
    local entity = self.character:get_vehicle()
    ...
    if entity.name == "tank" then
        collision_mask = {
            ...
            layers = { water_tile = true, lava_tile = true, cliff = true, player = true, car = true, train = true }
        }
    end
    ]]
...
function Moving_vehicle:finish()
    if self.character:get_vehicle() then
        self.character:get_vehicle().riding_state = { acceleration = defines.riding.acceleration.braking, direction = defines.riding.direction.straight }
    end
...
function Moving_vehicle:follow_path()
    local vehicle = self.character:get_vehicle()
...
function Moving_vehicle:check_stuck()
    ...
    if node == self.last_stuck_node then
        self.character:get_vehicle().orientation = get_orientation_to(self.character:get_vehicle().position, node.position)
    end
...
function Moving_vehicle:update()
    ...
    if not (self.character:get_vehicle()) then
        self:finish()
        return
    end
