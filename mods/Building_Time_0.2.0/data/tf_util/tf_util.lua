...
recursive_hack_tint = function(array, tint, mask_only)
  for k, v in pairs (array) do
    if type(v) == "table" then
      if is_sprite_def(v) and (not mask_only or util.has_flag(v.flags, "mask")) then
        v.tint = tint
        if v.hr_version then
          v.hr_version.tint = tint
        end
      end
      recursive_hack_tint(v, tint, mask_only)
    end
  end
end
...
util.has_flag = function(flags, flag)
...
recursive_hack_runtime_tint = function(prototype, value)
  for k, v in pairs (prototype) do
    if type(v) == "table" then
      if util.is_sprite_def(v) and util.has_flag(v.flags, "mask") then
        --error("HI")
