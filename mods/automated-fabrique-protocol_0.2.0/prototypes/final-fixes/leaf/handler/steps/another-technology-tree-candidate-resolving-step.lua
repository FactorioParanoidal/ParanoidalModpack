...
local function printInfoForAvailableForIngredientsTechnologyNames(available_technology_name_for_ingredients,
																  technology_name,
																  mode)
	...
	_table.each(basic_units,
		function(basic_unit)
			_table.remove_item(diff_units, basic_unit)
		end)
	...
	if _table.size(cyclingCandidates) > 0 then
		log(technology_name ..
			' contains in tree of ' ..
			available_technology_name_for_ingredients .. ' in following technologies ' ..
...
local function raiseAnotherTreesHandlingError(effect_ingredient_not_found_in_current_tree, technology_name, mode,
	...
	_table.each(available_technology_names_for_missed_ingredient,
		function(available_technology_name_for_missed_ingredient)
			printInfoForAvailableForIngredientsTechnologyNames(available_technology_name_for_missed_ingredient,
				technology_name,
				mode)
		end)
	...
	_table.each(available_technology_names_for_ingredients,
		function(available_technology_name_for_ingredients)
			printInfoForAvailableForIngredientsTechnologyNames(available_technology_name_for_ingredients,
				technology_name,
				mode)
		end)
...
local function handleNotFoundIngredentInAnotherTrees(effect_ingredient_not_found_in_current_tree, mode, technology_name)
	...
	for _, uncycled_tech_name_candidate in pairs(available_technology_names_for_ingredients) do
		if not techTreeUtil.haveTechnologyInTree(uncycled_tech_name_candidate, technology_name, mode)
		then
...
local function resolvingNotFoundIngredientsInAnotherTechnologyTree(mode, technology_name)
...
AnotherTechnologyTreeResolvingStep.evaluate = function(technology_name, mode)
	...
	resolvingNotFoundIngredientsInAnotherTechnologyTree(mode, technology_name)
	...
	_table.each(dependencies,
		function(dependency_name)
			AnotherTechnologyTreeResolvingStep.evaluate(dependency_name, mode)
		end)
end
