...
local function updateFactorissimoMod(mode)
	...
	techUtil.reset_prerequisites_for_technology(connection_chest_tech, { "factory-architecture-t1", "logistics" }, mode)
	...
	techUtil.reset_prerequisites_for_technology(
		...
		{ "factory-architecture-t1", "coal-wooden-fluid-handling" },
		mode
	)
	...
	if mods["bobpower"] then
		techUtil.add_prerequisites_to_technology(
			technologies["factory-connection-type-heat"],
			{ "bob-heat-pipe-1" },
			mode
		)
	end
