...
for k,v in pairs(data.raw.recipe) do
	...
	local allowDecomposition = 
		...
		(
			...
			v.category ~= "ore-sorting-t1" and
			v.category ~= "ore-sorting-t2" and
			v.category ~= "ore-sorting-t3" and
			v.category ~= "ore-sorting-t4" and
			v.category ~= "crystallizing" and
			v.category ~= "ore-sorting" and
			v.category ~= "ore-sorting-t1-5" and
			v.category ~= "ore-sorting-t3-5" and
			v.category ~= "filtering" and
			...
			v.category ~= "ore-processing" and
			v.category ~= "pellet-pressing" and
			v.category ~= "chemical-smelting" and
			v.category ~= "powder-mixing" and
			v.category ~= "blast-smelting" and
			v.category ~= "smelting" and
			v.category ~= "induction-smelting" and
			v.category ~= "induction-smelting-2" and
			v.category ~= "induction-smelting-3" and
			v.category ~= "strand-casting" and
			v.category ~= "strand-casting-2" and
			v.category ~= "casting" and
			v.category ~= "sintering" and

			-- no petrochem at all unless set by exceptions. FUBAR is the least you can call it.
			v.category ~= "chemistry" and
			v.category ~= "liquifying" and
			v.category ~= "petrochem-electrolyser" and
			v.category ~= "petrochem-air-filtering" and
			v.category ~= "steam-cracking" and
			v.category ~= "gas-refining" and
			v.category ~= "oil-processing" and
			v.category ~= "advanced-chemistry" and

			-- most of the furnace recipes are part of smelting, with the rest being best left off anyway. few exceptions are set below.
			v.category ~= "mixing-furnace" and
			v.category ~= "chemical-furnace" and

			-- water processing. not really necessary overall
			v.category ~= "thermal-bore" and
			v.category ~= "thermal-extractor" and
			v.category ~= "water-treatment" and
			v.category ~= "salination-plant" and
			v.category ~= "washing-plant" and
			v.category ~= "cooling" and
			v.category ~= "petrochem-boiler" and
			...
			v.category ~= "bio-processing" and
			v.category ~= "bio-processor" and
			v.category ~= "bio-processing-wood" and
			v.category ~= "angels-arboretum" and
			v.category ~= "angels-arboretum-temperate" and
			v.category ~= "angels-arboretum-swamp" and
			v.category ~= "angels-arboretum-desert" and
			v.category ~= "angels-tree" and
			v.category ~= "angels-tree-temperate" and
			v.category ~= "angels-tree-swamp" and
			v.category ~= "angels-tree-desert" and
			v.category ~= "seed-extractor" and
			v.category ~= "basic-farming" and
			v.category ~= "temperate-farming" and
			v.category ~= "swamp-farming" and
			v.category ~= "desert-farming" and
			v.category ~= "nutrient-extractor" and
			v.category ~= "bio-pressing" and
			v.category ~= "bio-refugium-fish" and
			v.category ~= "bio-refugium-puffer" and
			v.category ~= "bio-refugium-biter" and
			v.category ~= "bio-butchery" and
			v.category ~= "bio-hatchery" and

			--deadlock (i like those mods, crating especially)
			v.category ~= "crating" and
			v.category ~= "packing" and
			v.category ~= "stacking" and

			--several edge cases we can handle in bulk:
			string.find(k,"-sawing") == nil and		--wood production from tree cutting
			string.find(k, "-converting") == nil and	-- metal roll to plate and roll to wire recipes. plate to wire direct recipes taken care of in exceptions
			string.find(k, "-plate") == nil and 		-- bob plate recipes (blocked in angel)
		...
		--	string.find(k, "diamond") == nil and
			...
			string.find(k, "reprocessing") == nil and
			string.find(k, "enrichment") == nil
		)
