...
require "__mferrari_lib__/speech-bubbles"
...
local sub_inputs =  {
 ...
 ['hs_rockets'] = 		{"hs_rockets_launched","hs_item_launched", "hs_item_launched_here"},
 ['hs_production_m'] =  {"hs_production","hs_production_f","hs_production_p","hs_production_ratio","hs_production_ratio_f"},  --hs_production_p= power
...
local pk_item_list =   {"hs_logistic","hs_production","hs_consumption","hs_item_launched","hs_item_launched_here","hs_production_ratio"}
...
storage.signs = storage.signs or {}
storage.players = storage.players or {} -- to control opened guis
storage.last_death_tick = storage.last_death_tick or 0
storage.force_research = storage.force_research or {}

storage.inputs = table.deepcopy(inputs)
storage.sub_inputs = table.deepcopy(sub_inputs)
--[[storage.signs[1] = {entity =
                   text=
				   icon = 
				   add_tag=
				   maptag=}]]
--[[if script.active_mods['shield-projector'] then 
	storage.animation = 'hs_hologram_animated'
	else]]
storage.animation = 'hs_hologram'
	--end
if script.active_mods['WindSpeedChanging']  or script.active_mods['nullius'] or script.active_mods['windturbines-redux']  then add_list(storage.inputs, "hs_wind_speed") end
if script.active_mods['Warp-Drive-Machine'] then add_list(storage.inputs, "hs_warpdrive") add_list(storage.inputs, "hs_wind_speed") end


storage.item_elem_type="item"
if script.active_mods['quality'] then storage.item_elem_type="item-with-quality" end

storage.SA_items_launched = storage.SA_items_launched or {}

--[[
storage.quality_opts = {}
if script.active_mods['quality'] then
for name, proto in pairs (prototypes.quality) do
	storage.quality_opts[name] = {"quality-name."..name}
	end
	end
]]
end
...
storage.opt_animation = settings.global["hs-opt-animation"].value
ValidateAnimations()
end
...
for UN, Sign_Data in pairs (storage.signs) do
	local animation = Sign_Data.animation
	local entity    = Sign_Data.entity
	--if animation and animation.valid and (not storage.opt_animation) then animation.destroy() Sign_Data.animation=nil end
	if not (animation and animation.valid) then --storage.opt_animation
		--[[if storage.animation == 'hs_hologram_animated' then
			animation = rendering.draw_animation {
					animation = 'hs_hologram_animated',
					x_scale=0.07,y_scale=0.35,
					target = {entity=entity,offset = {0, -1.1}},
...
local valid = false
if in_list(storage.inputs, input) then valid=true 
else
for g,tab in pairs (storage.sub_inputs) do 
...
function ValidateAllSigns()
	local function validate_quality(icon)
		if icon and icon.quality then 
			if not prototypes.quality[icon.quality] then icon.quality=nil end
			end		
		end
for UN, Sign_Data in pairs (storage.signs) do
	local entity = Sign_Data.entity
	...
	validate_quality(icon)
	-- Manage INPUTS
	if input then 
		if  in_list(pk_signal_list, input) then  --'hs_signal'
			local signal = Sign_Data.input_signal
			if signal then 
				validate_quality(signal)
				if signal.type=='item' then if not prototypes.item[signal.name] then Sign_Data.input=nil end
					elseif signal.type=='fluid' then if not prototypes.fluid[signal.name] then Sign_Data.input=nil  end
					elseif signal.type=='virtual' then if not prototypes.virtual_signal[signal.name] then Sign_Data.input=nil  end
					elseif signal.type=='recipe' then if not prototypes.recipe[signal.name] then Sign_Data.input=nil  end
					end
				end
		elseif in_list(pk_item_list, input) then 
			...
			if item then 
				if type(item)=="table" and (not (prototypes.item[item.name])) then Sign_Data.input=nil 
				elseif type(item)=="string" and (not (prototypes.item[item])) then Sign_Data.input=nil end
				end
		elseif in_list(pk_fluid_list, input) then 
			local fluid = Sign_Data.input_fluid
			if not (fluid and prototypes.fluid[fluid]) then Sign_Data.input=nil end
		elseif input=='hs_kill_count' then 
			local killed = Sign_Data.input_entity
			if not (killed and prototypes.entity[killed]) then Sign_Data.input=nil end

        elseif not is_valid_input(input) then Sign_Data.input=nil    
		
		elseif in_list(sub_inputs['hs_warpdrive'], input) then
			if not remote.interfaces["WDM"] then Sign_Data.input=nil  end
		end
	
	end
	-- MAP TAG
	if icon and icon.type and icon.name then 
			if icon.type=='item' then if not prototypes.item[icon.name] then Sign_Data.icon=nil end
			elseif icon.type=='fluid' then if not prototypes.fluid[icon.name] then Sign_Data.icon=nil end
			elseif icon.type=='virtual' then if not prototypes.virtual_signal[icon.name] then Sign_Data.icon=nil end		
			end
		end
	end
end
...
local Sign_Data = storage.signs[UN] or {}

if not Sign_Data.entity then Sign_Data.entity=entity end

storage.players[player.name] = Sign_Data

--gui.clear()
if gui.frame_holo_sign then gui.frame_holo_sign.destroy() end

local frame = add_gui(player.gui.screen,{type = "frame", name = "frame_holo_sign", direction = "vertical"})
add_gui_frame_title_and_close_bt(frame,{'','[img=item/hs_holo_sign]',{'holographic_sign'}})
...
local txt = add_gui(tab, {type="textfield", name='hs_text_field' , text =Sign_Data.text, icon_selector=true} )
local ico = add_gui(tab, {type="choose-elem-button", name='hs_icon_bt', elem_type="signal"} )  --elem_type ='item' item='steel-plate'    elem_value
...
add_gui(tab, {type="sprite-button", sprite = "utility/close", name='bt_destroy_my_2parent', style = "back_button" }) --style='rounded_button'style="tool_button"
...
local Sign_Data = storage.players[player.name]
...
local function on_gui_value_changed(event)  --slider
...
local Sign_Data = storage.players[player.name]
	...
	--game.print(input)
	if not input   then  
		for i=1, #storage.inputs do
			frame.add{type="radiobutton",name=storage.inputs[i], caption={storage.inputs[i]}, state=iif(storage.inputs[i]==input, true, false) }.style.font = "default-bold"
			end
		else
		...
		if storage.sub_inputs[input] then 
			for i=1, #storage.sub_inputs[input] do
				frame.add{type="radiobutton",name=storage.sub_inputs[input][i], caption={storage.sub_inputs[input][i]}, state=iif(storage.sub_inputs[input][i]==input, true, false) }.style.font = "default-bold"
				end			
			end
		
			if in_list(pk_signal_list, input) then
				local ico = add_gui(frame, {type="choose-elem-button", name='hs_choose_input', elem_type="signal", tags={input=input}} )  --elem_type ='item' item='steel-plate'    elem_value
				ico.elem_value= Sign_Data.input_signal
			
				elseif (in_list(pk_item_list, input)) then
					local ico = add_gui(frame, {type="choose-elem-button", name='hs_choose_item', elem_type=storage.item_elem_type, tags={input=input}} )  --elem_type ='item' item='steel-plate'    elem_value
					ico.elem_value= Sign_Data.input_item

				elseif (in_list(pk_entity_list, input)) then
					local ico = add_gui(frame, {type="choose-elem-button", name='hs_choose_entity', elem_type="entity", tags={input=input}, elem_filters={{type = "entity", filter = "entity-with-health"}}  } )  --elem_type ='item' item='steel-plate'    elem_value
					ico.elem_value= Sign_Data.input_entity

				elseif (in_list(pk_fluid_list,input)) then
					local ico = add_gui(frame, {type="choose-elem-button", name='hs_choose_fluid', elem_type="fluid", tags={input=input}} )  --elem_type ='item' item='steel-plate'    elem_value
...
if gui.get_mod()=='holographic_signs' then
	local player = game.players[event.player_index]
	--if in_list(storage.inputs,gui.name) or in_list(storage.sub_inputs[1],gui.name) or in_list(storage.sub_inputs[2],gui.name) then
...
local Sign_Data = storage.players[player.name]
	local textbox = gui.add{type="text-box", name="hs_textbox",text=Sign_Data.textbox,icon_selector=true}
...
if entity and entity.valid then 
	...
	if big_hologram_render then 
		if big_hologram_render.valid then big_hologram_render.destroy() end
...
if storage.all_turned_off==true or state=='left' then is_it_on = false end
	...
	if input then 
			if input=='hs_signal' then 
				...
				if signal and signal.name then 
					ipt = format_number (entity.get_signal(signal,defines.wire_connector_id.circuit_green,defines.wire_connector_id.circuit_red)) --get signal
				else-- new: if left blank, can read first signal with icon	
					local signals = entity.get_signals(defines.wire_connector_id.circuit_green,defines.wire_connector_id.circuit_red)
					if signals then 
					for _, asignal in pairs (signals) do
						local signal=asignal.signal
						Sign_Data.icon = signal
						ipt = asignal.count
						break
						end
					end
			...
			elseif input=='hs_hologram_image' then 
				...
				if signal then 
					img_scale = 2
					holo_img = translate_element_icon(signal)
					end
			...
			elseif in_list({'hs_production','hs_consumption','hs_item_launched',"hs_item_launched_here"},input) then 
				local item = Sign_Data.input_item
				if item then 
					local count
						if input=='hs_production' then count = force.get_item_production_statistics(entity.surface).get_input_count(item)
						elseif input=='hs_consumption' then count  = force.get_item_production_statistics(entity.surface).get_output_count(item)
						elseif input=='hs_item_launched' then 
							local it_name= item.name or item
							count  = force.get_item_launched(it_name)
						elseif input=="hs_item_launched_here" then 
							local it_name= item.name or item
							count = 0
							if storage.SA_items_launched[force.name] and storage.SA_items_launched[force.name][entity.surface.name]
							   and storage.SA_items_launched[force.name][entity.surface.name][it_name] then 
									count =storage.SA_items_launched[force.name][entity.surface.name][it_name]
							   end
						end
			...
			elseif input=='hs_production_ratio' then 
				...
				if item then 
					...
					local count = force.get_item_production_statistics(entity.surface).get_flow_count{name=item,category="input", precision_index=sub_option}
					local cons = force.get_item_production_statistics(entity.surface).get_flow_count{name=item,category="output", precision_index=sub_option}
					local ratio = 0
			...
			elseif input=='hs_production_ratio_f' then 
				...
				if fluid then 
					local sub_option = Sign_Data.sub_option
					sub_option = get_precision_index(sub_option)
					local count = force.get_fluid_production_statistics(entity.surface).get_flow_count{name=fluid,category="input", precision_index=sub_option}
					local cons = force.get_fluid_production_statistics(entity.surface).get_flow_count{name=fluid,category="output", precision_index=sub_option}
					local ratio = 0
					if cons>0 then ratio=count/cons end
			...
			elseif input=='hs_production_f' or input=='hs_consumption_f' then 
				...
				if fluid then 
					local count
						if input=='hs_production_f' then count = force.get_fluid_production_statistics(entity.surface).get_input_count(fluid)
						elseif input=='hs_consumption_f' then count  = force.get_fluid_production_statistics(entity.surface).get_output_count(fluid)
						end
					ipt = my_format_number(math.floor(count), 1000000)
					end

			elseif input=='hs_production_p' or input=='hs_consumption_p' then 
				local pole = entity.surface.find_entities_filtered{type='electric-pole', force=force, position=entity.position, radius=3, limit=1}
				if pole[1] then 
					...
					for n,v in pairs (NS.output_counts) do 
						prod = prod + NS.get_flow_count{name=n,category="output", precision_index=defines.flow_precision_index.five_seconds, sample=1}
						end
					-- cons
					for n,v in pairs (NS.input_counts) do 
						cons = cons + NS.get_flow_count{name=n,category="input", precision_index=defines.flow_precision_index.five_seconds, sample=1}
						end
			...
			elseif input=='hs_player_casualties' then 
				ipt = force.get_kill_count_statistics(entity.surface).get_output_count('character')			
			elseif input=='hs_enemy_evolution' then 
				ipt = format_force_evolution(game.forces.enemy,entity.surface) -- .. '%'
			elseif input=='hs_kill_count' then 
				local killed = Sign_Data.input_entity
				if killed then ipt = format_number(force.get_kill_count_statistics(entity.surface).get_input_count(killed)) end
			...
			elseif input=='hs_time_from_last_death' then 
				ipt = format_time_from_tick(storage.last_death_tick)
...
local pt = text
if icon then --and (not plain_text) 
	if type(icon)=="table" then
		local quality
		if icon.quality then 
			pt ="[item=" ..icon.name.. ",quality="..icon.quality..']'
		else
			pt = '[img='
			if icon.type then 
				if icon.type=='virtual' then pt = pt .. 'virtual-signal' 
					else pt = pt .. icon.type 
					end
				else pt = pt .. 'item'
				end
			pt = pt .. '/' .. icon.name..']'
...
if (pt~='' or holo_img) and is_it_on then 
	if pt~='' then 
		if plain_text then 
		if the_plain_text and the_plain_text.valid then 
			the_plain_text.color=color
			the_plain_text.text = pt
			else 
			the_plain_text = rendering.draw_text{
							text=pt, 
							surface=entity.surface, 
							target={entity=entity,offset={-1,-1}},
							color=color,
							use_rich_text=true}
	...
	if holo_img then
		if big_hologram_render and big_hologram_render.valid then 
			if big_hologram_render.sprite ~= holo_img then big_hologram_render.sprite=holo_img end
			else
			big_hologram_render = rendering.draw_sprite {  
					sprite = holo_img,
					x_scale=img_scale,y_scale=img_scale,
					target={entity=entity,offset={0,-2}},
					surface = entity.surface}
	...
	if the_plain_text then destroy_plan_text =true end
	if animation and animation.valid then animation.destroy() Sign_Data.animation=nil end
	if big_hologram_render and big_hologram_render.valid then big_hologram_render.destroy() Sign_Data.big_hologram_render=nil end
	end

if destroy_plan_text then 
	the_plain_text.destroy()
...
if add_tag and is_it_on then 
	local ico 
	if icon then ico = translate_element_icon(icon, true) end
	if ico or (text~='') then 
...
function update_force_research_durations(force, current_research)
  local force_table = storage.force_research[force.name]
...
if not storage.force_research[force.name] then storage.force_research[force.name] = {} end
local last_check = storage.force_research[force.name].last_research_progress_tick or 0
if game.tick - last_check  > 60 then 
    local current_research = force.current_research
    if current_research then
       update_force_research_durations(force, current_research)
	   else
	   storage.force_research[force.name].duration_text = "[img=infinity]"
	   end
  end
return storage.force_research[force.name].duration_text
end
...
function Copy_Sign_Data(Entity, Sign_Data)
	...
	if storage.signs[NUN] then 
		if storage.signs[NUN].the_plain_text then 
			storage.signs[NUN].the_plain_text.destroy()
			storage.signs[NUN].the_plain_text=nil
			end
		if storage.signs[NUN].animation then 
			storage.signs[NUN].animation.destroy()
			storage.signs[NUN].animation=nil
			end			
		if storage.signs[NUN].big_hologram_render then 
			storage.signs[NUN].big_hologram_render.destroy()
			storage.signs[NUN].big_hologram_render=nil
			end

		
		if storage.signs[NUN].maptag and storage.signs[NUN].maptag.valid then storage.signs[NUN].maptag.destroy() end
		end
	storage.signs[NUN] = Sign_Data
	apply_holo_sign(Sign_Data)
end
...
if storage.signs[UN] then 
	local Sign_Data = table.deepcopy(storage.signs[UN])
...
script.on_event(defines.events.on_built_entity , ghost_revived_event)
...
script.on_event(defines.events.on_space_platform_built_entity, ghost_revived_event)
...
--local player = game.players[event.player_index]
if gui.get_mod()=='holographic_signs' then
	if in_list({'hs_choose_input','hs_choose_item','hs_choose_fluid'},gui.name) then
		local value=gui.elem_value  --changed on 2.0
		Log(value)
		if value and (type(value)=="string" or value.name) then 
			local elem_type=gui.elem_type
			if elem_type =="signal" then 
				if prototypes.item[value.name] then elem_type="item" 
				elseif prototypes.fluid[value.name] then elem_type="fluid" 
				elseif prototypes.entity[value.name] then elem_type="entity" 
				elseif prototypes.recipe[value.name] then elem_type="recipe" 
				else elem_type="virtual" end
				end
			if elem_type=="item-with-quality" then elem_type="item" end
			local main_ico = gui.parent.parent.tab1.hs_icon_bt --adds to print icon if empty
			if not (main_ico and main_ico.elem_value) then
				if not (gui.tags and gui.tags.input and gui.tags.input=="hs_hologram_image") then
				main_ico.elem_value={type=elem_type, name=value.name or value, quality=value.quality}
				end end
			end
		end
...
for UN, Sign_Data in pairs (storage.signs) do
	local entity = Sign_Data.entity
	if entity and entity.valid then
		local green = entity.get_signal({type="virtual",name='signal-green'}, defines.wire_connector_id.circuit_red)==1 
						or entity.get_signal({type="virtual",name='signal-green'}, defines.wire_connector_id.circuit_green)==1
		local red   = entity.get_signal({type="virtual",name='signal-red'}, defines.wire_connector_id.circuit_red)==1
		 				or entity.get_signal({type="virtual",name='signal-red'}, defines.wire_connector_id.circuit_green)==1
...
-- only used for space age -- counting items launched
...
storage.SA_items_launched[force.name] = storage.SA_items_launched[force.name] or {}
storage.SA_items_launched[force.name][surface.name] = storage.SA_items_launched[force.name][surface.name] or {}


local contents = inventory.get_contents()
    for _, stack in pairs(contents) do
		local total = storage.SA_items_launched[force.name][surface.name][stack.name] or 0
		total=total + stack.count
		storage.SA_items_launched[force.name][surface.name][stack.name]=total
		end
end
end)
