...
if settings.get_player_settings(player)["charxpmod_print_xp_user"].value and player.character and player.character.valid then
	rendering.draw_text{text="+" .. RPG_format_number(XP) .." XP",
		color = colors.yellow, surface=player.character.surface, target={entity=player.character, offset={-2+math.random()*4,-3 + math.random()*2}}  , time_to_live=70}
	end
end
end
...
for pname, v in pairs (storage.personalxp.rocketsXP_count) do
	if type(v)=="number" then storage.personalxp.rocketsXP_count = {nauvis = v} end
	end
end
...
Topframe.add{name="btcharxp", type="sprite-button", sprite = "entity/character", tooltip = {"panel-title", player.name}, style = mod_gui.top_button_style} -- "mod_gui_button"}
...
function XP_Time_upd()
	...
	if QP>0 then storage.XP_AVG_PLAYTIME[name] = TT/QP end
	end
	end
end
...
ratioXP = math.floor(ratioXP*100)
		...
		local tabStats = tabScroll.add{type = "table", name = "tabStats", column_count = 1}
			...
			tabStats.add{type="label", name='STT2', caption={'xp_deaths',storage.personalxp.Death[name]}}.style.width = wd
...
if XP >= storage.max_xp then NextLtxt = 'MAX' end
...
for A=1,#storage.Player_Attributes do
	...
	if storage.RPG_Bonus[attrib]==0 then enabled = false end 
...
frame.add{type = "line"}
local pickbutton = frame.add{type = "checkbox", name = "cb_pick_extender", caption={'xp_opt_Pick_Extender'}, state = storage.personalxp.opt_Pick_Extender[name]}
frame.add{type = "line"}
end
...
if XP > storage.max_xp then XP = storage.max_xp end
local NextLevel = storage.xp_table[Level]
...
if Level<storage.setting_max_player_level then
	local NextLevel = storage.xp_table[Level]
	local PriorLVXP = 0
	if Level>1 then PriorLVXP = storage.xp_table[Level-1] end
...
if not frame then
	...
	local tagfield = title_table.add{type="textfield", name="ctag_field", text=player.tag, icon_selector=true}
	tagfield.style.width=200
	tagfield.visible=false
	
	local edittag = title_table.add{name="rpg_bt_edittag",  type="sprite-button", sprite = "utility/rename_icon", style = "shortcut_bar_button_small", tooltip={'player_tag'}}
...
if     stat=='character_crafting_speed_modifier' then 
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Craft_Speed[name] * storage.RPG_Bonus['LV_Craft_Speed']/100 
elseif stat=='character_mining_speed_modifier' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Mining_Speed[name] * storage.RPG_Bonus['LV_Mining_Speed']/100 
elseif stat=='character_running_speed_modifier' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Run_Speed[name] * storage.RPG_Bonus['LV_Run_Speed']/100 
elseif stat=='character_build_distance_bonus' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Reach_Dist[name]* storage.RPG_Bonus['LV_Reach_Dist']
...
elseif stat=='character_inventory_slots_bonus' then
   player.character[stat] = player.character[stat] +  storage.personalxp.LV_Inv_Bonus[name] * storage.RPG_Bonus['LV_Inv_Bonus'] 
elseif stat=='character_trash_slot_count_bonus' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_InvTrash_Bonus[name]* storage.RPG_Bonus['LV_InvTrash_Bonus'] 
elseif stat=='character_maximum_following_robot_count_bonus' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Robot_Bonus[name]* storage.RPG_Bonus['LV_Robot_Bonus'] 
elseif stat=='character_health_bonus' then
   player.character[stat] = player.character[stat] + storage.personalxp.LV_Health_Bonus[name]* storage.RPG_Bonus['LV_Health_Bonus'] 
elseif stat=='character_item_pickup_distance_bonus' then
	 if storage.personalxp.opt_Pick_Extender[name] then 
        player.character[stat] = player.character[stat] + storage.personalxp.LV_Reach_Dist[name]* storage.RPG_Bonus['LV_Reach_Dist'] end
...
for A=1,#storage.Player_Attributes do
	...
	if btname=='btLVU_global.personalxp.'.. attrib then 
		...
		if btname=='btLVU_global.personalxp.LV_Craft_Speed' then
			player.character.character_crafting_speed_modifier = player.character.character_crafting_speed_modifier + storage.RPG_Bonus[attrib]/100 end 
		if btname=='btLVU_global.personalxp.LV_Mining_Speed' then
			player.character.character_mining_speed_modifier = player.character.character_mining_speed_modifier + storage.RPG_Bonus[attrib]/100 end 
		if btname=='btLVU_global.personalxp.LV_Run_Speed' then
			player.character.character_running_speed_modifier = player.character.character_running_speed_modifier + storage.RPG_Bonus[attrib]/100 end 
		...
		if btname=='btLVU_global.personalxp.LV_Reach_Dist' then
			...
			player.character.character_resource_reach_distance_bonus = player.character.character_resource_reach_distance_bonus + storage.RPG_Bonus[attrib] end
			
		if btname=='btLVU_global.personalxp.LV_Inv_Bonus' then
			player.character.character_inventory_slots_bonus = player.character.character_inventory_slots_bonus + storage.RPG_Bonus[attrib] end 

		if btname=='btLVU_global.personalxp.LV_InvTrash_Bonus' then
			player.character.character_trash_slot_count_bonus = player.character.character_trash_slot_count_bonus + storage.RPG_Bonus[attrib] end 

		if btname=='btLVU_global.personalxp.LV_Robot_Bonus' then
			player.character.character_maximum_following_robot_count_bonus = player.character.character_maximum_following_robot_count_bonus + storage.RPG_Bonus[attrib] end 

		if btname=='btLVU_global.personalxp.LV_Health_Bonus' then
			player.character.character_health_bonus = player.character.character_health_bonus + storage.RPG_Bonus[attrib] end 
    ...
    while i <= len do
        ...
        if char == "[" then
            ...
            if close_pos then
                ...
                -- Se for bloco válido (tem "=")
                if content:find("=") then
                    -- Só adiciona se ainda cabe no limite
                    if count + 1 > max_chars then break end

                    table.insert(result, text:sub(i, close_pos))
                    count = count + 1
                    i = close_pos + 1
                else
                    -- Não é bloco válido → trata como caractere normal
    ...
    elseif (name == "rpg_bt_edittag")then
		...
		if tagfield.visible then
			...
			if player.tag~=text then 
				...
				if string.find(text,"%[item=") then text=string.gsub(text, "%[item=", "[img=item/") end
				if string.find(text,"%[fluid=") then text=string.gsub(text, "%[fluid=", "[img=fluid/") end
				if string.find(text,"%[virtual-signal=") then text=string.gsub(text, "%[virtual-signal=", "[img=virtual-signal/") end
				if string.find(text,"%[entity=") then text=string.gsub(text, "%[entity=", "[img=entity/") end
				if string.find(text,"%[recipe=") then text=string.gsub(text, "%[recipe=", "[img=recipe/") end

				text = limit_text_with_icons(text, 20)
				player.tag = text
...
if entity.type == 'character' then XP = XP * 4
	elseif entity.type == 'unit' then
		...
		if player then storage.personal_kill_units[plname]=storage.personal_kill_units[plname]+1 end
		elseif entity.type == 'unit-spawner' then
		XP = XP * 2
		storage.kills_spawner[nforce] = storage.kills_spawner[nforce] +1
		if player then storage.personal_kill_spawner[plname]=storage.personal_kill_spawner[plname]+1 end
		elseif entity.type == 'turret' then
		storage.kills_worms[nforce] = storage.kills_worms[nforce] +1
		if player then storage.personal_kill_turrets[plname]=storage.personal_kill_turrets[plname]+1 end
		XP = XP * 1.5
		end
...
--	if XP > 999999 then XP=999999 end
	...
	local teamxp = true
		if plname then
			...
			if (not storage.last_magical_effect_from[player.name]) or storage.last_magical_effect_from[player.name]+60*stg_magic_interval<game.tick then
...
if player.character and player.character.valid and player.character.surface==entity.surface and  
	player.controller_type==defines.controllers.character then  -- new because of remote view
	...
	if LV > 0 then
		...
		if math.random(100)<=chance then 
			...
			if LV>12 then table.insert(weap,"rpg_hadouken") end	
			if LV>15 then table.insert(weap,"rpg_fireaball") end
			
			if LV>5  then table.insert(weap,'defender') end
			if LV>10 then table.insert(weap,'distractor') end
			if LV>17 then table.insert(weap,'destroyer') end
			...
			if w=='rpg-magic-shock' then ShockNear(entity,player,LV)
				...
				if in_list({"defender","distractor","destroyer"},w) then po=entity.position end
				player.surface.create_entity{name=w, target=entity, target_position=entity.position,  position=po, force=player.force, speed=0.5}  --0.4
...
for x=1,math.random(math.min(LV,20))+1 do
	...
	if entity.valid then
		...
		if entity.valid and player.character.surface==entity.surface then entity.damage(5 + LV*5,player.force , 'electric', player.character) end
		end
	end
end
end
...
if killer and killer.valid and storage.kills_units[force.name] and event.entity.force~=game.forces.neutral then
	if event.entity.prototype and event.entity.prototype.get_max_health() and (not force.get_friend(event.entity.force)) then
...
if not killer then
 ...
 {{filter = "type", type = "unit"}, {filter = "type", type = "unit-spawner"}, 
		...
		{filter = "type", type = "character"},{filter = "type", type = "character"},
...
if storage.setting_print_critical then
	...
	rendering.draw_text{text="[img=utility/ammo_damage_modifier_constant] ".. math.ceil(damage), --
		color = colors.red, surface=entity.surface, target=p, time_to_live=120,use_rich_text=true, scale=2}
end
end
...
if ent.type=='tree' then XP=1 else XP=2 end
...
storage.personalxp.XP[playername] = storage.personalxp.XP[playername] - math.ceil(storage.personalxp.XP[playername]*Perc/100) 
XP_UPDATE_tick()
end
