...
function post_process.index_properties()
	...
	local function index_subgroups()
		...
		-- Entities
		for item_type, _ in pairs(OSM.dictionary.item) do -- from item place_result
			for _, item in pairs(data.raw[item_type]) do
		...
		for entity_type, _ in pairs(OSM.dictionary.entity) do -- from entity placeable_by
			for _, entity in pairs(data.raw[entity_type]) do		
				...
				if technology.effects then
					for i, effect in pairs(technology.effects) do
						if effect.type == "unlock-recipe" and effect.recipe == recipe.name then
							...
							table.insert(OSM.log.warnings, "Warning: "..'"OSM-Lib Post Process Script"'..": Removed unlock from technologies for: "..'"'..recipe.name..'"'.." because it was disabled by: "..'"'..mod_name..'"')
						end
					end
				end
			end
...
--			prototype.icon_size = nil		-- used by dark_background_icon
	...
	-- Scan prototypes
	for _, sub_type in pairs(OSM.dictionary.sub_types) do
