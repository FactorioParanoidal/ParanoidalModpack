...
if settings.startup['picker-disable-decorations'].value then
    for _, deco in pairs(data.raw['optimized-decorative']) do deco.autoplace = nil end
end
...
--]] --
if settings.startup['picker-clean-tree-burning'].value then Data('fire-flame-on-tree', 'fire').tree_dying_factor = 1 end

-- (( RoundUp ))--
if settings.startup['picker-roundup'].value then
    local tiles = {
        'concrete', 'stone-path', 'hazard-concrete-left', 'hazard-concrete-right', 'refined-concrete',
        'refined-hazard-concrete-left', 'refined-hazard-concrete-right'
    }

    for _, tile in pairs(tiles) do Data(tile, 'tile').decorative_removal_probability = 1 end
end
