...
--	game.print(entity.type)
	...
	--DEPRECATED FOR NOW: handled on miner rotation instead
...
--	if entity.type == "simple-entity-with-force" or entity.type == "mining-drill" and not added then
...
local function move_lamp(event)
	...
	if entity.type == "mining-drill" and entity.name ~= "burner-mining-drill" then
		-- check the miner already has a lamp, if not, add one
		if global.assemblerlamps[entity.unit_number] == nil then
			add_lamp(entity)
		end
...
function remove_lamp(event)
	...
	if assemblerlamp then 
		if assemblerlamp.valid then -- safety check to avoid crash
	...
	--cleanup any orphaned assemblerlamps (ie created by weirdness by other mods)
	-- only underneath the removed building, and only on player entities (ie, not biters)
	if entityForce == "player" then
		for _, surface in pairs(game.surfaces) do
			for _, assemblerlamp in pairs(surface.find_entities_filtered{position = entityposition, radius = 1,name="assembler-lamp"}) do
				assemblerlamp.destroy()
			end
		end
	end
