...
local function verify_all_configs()
  ...
  for _, config in pairs(global["config-tmp"]) do verify_config(config) end
  for _, storage in pairs(global.storage) do
    for _, config in pairs(storage) do verify_config(config) end
  end
end

Event.register(Event.core_events.on_configuration_changed, function(event)
  if not event or not event.mod_changes then return end
  verify_all_configs()
end)
...
Event.register(defines.events.on_pre_player_removed, function(event)
...
Event.register("upgrade-planner-hide", function(event)
  ...
  if button_flow["upgrade_planner_config_button"] then
    button_flow["upgrade_planner_config_button"].visible =
        not button_flow.upgrade_planner_config_button.visible
    return
  end
...
Event.register(defines.events.on_player_dropped_item, function(event)
  ...
  if (stack.name == "upgrade-builder") then event.entity.destroy() end
end)
