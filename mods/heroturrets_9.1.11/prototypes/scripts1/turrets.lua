...
local local_get_damage_table = function()
	...
	if ranks == 4 then
		damage_table = {
			heroturrets.defines.turret_levelup_damage_1,
			heroturrets.defines.turret_levelup_damage_2,
			heroturrets.defines.turret_levelup_damage_3,
			heroturrets.defines.turret_levelup_damage_4,
			heroturrets.defines.turret_levelup_damage_5,
			heroturrets.defines.turret_levelup_damage_6,
			heroturrets.defines.turret_levelup_damage_7,
			heroturrets.defines.turret_levelup_damage_8,
			heroturrets.defines.turret_levelup_damage_9,
			heroturrets.defines.turret_levelup_damage_10
		}
	else		
		damage_table = {}
		local diff = (heroturrets.defines.turret_levelup_damage_10-heroturrets.defines.turret_levelup_damage_1)/#local_get_names()
		local current = heroturrets.defines.turret_levelup_damage_1
		for k=1, #local_get_names()-1 do
			table.insert(damage_table,current)
			current = math.floor(current + diff)
		end
		table.insert(damage_table,heroturrets.defines.turret_levelup_damage_10)
	end
	return damage_table 
end
...
local local_get_kill_table = function()
	...
	if ranks == 4 then
		kill_table = {
			heroturrets.defines.turret_levelup_kills_1,
			heroturrets.defines.turret_levelup_kills_2,
			heroturrets.defines.turret_levelup_kills_3,
			heroturrets.defines.turret_levelup_kills_4,
			heroturrets.defines.turret_levelup_kills_5,
			heroturrets.defines.turret_levelup_kills_6,
			heroturrets.defines.turret_levelup_kills_7,
			heroturrets.defines.turret_levelup_kills_8,
			heroturrets.defines.turret_levelup_kills_9,
			heroturrets.defines.turret_levelup_kills_10
		}
	else
		kill_table = {}
		log("Building kill table")
		local diff = (heroturrets.defines.turret_levelup_kills_10-heroturrets.defines.turret_levelup_kills_1)/#local_get_names()
		local current = heroturrets.defines.turret_levelup_kills_1
		for k=1, #local_get_names()-1 do
			table.insert(kill_table,current)
			current = math.floor(current + diff)
		end
		table.insert(kill_table,heroturrets.defines.turret_levelup_kills_10)
	end
...
local local_turret_added = function(entity,event)	
	...
	--[[
		local levelup_4 = heroturrets.defines.turret_levelup_kills_4 * multiplier
		local levelup_3 = heroturrets.defines.turret_levelup_kills_3 * multiplier
		local levelup_2 = heroturrets.defines.turret_levelup_kills_2 * multiplier
		local levelup_1 = heroturrets.defines.turret_levelup_kills_1 * multiplier
		local levelup_damage_4 = heroturrets.defines.turret_levelup_damage_4 * multiplier
		local levelup_damage_3 = heroturrets.defines.turret_levelup_damage_3 * multiplier
		local levelup_damage_2 = heroturrets.defines.turret_levelup_damage_2 * multiplier
		local levelup_damage_1 = heroturrets.defines.turret_levelup_damage_1 * multiplier
	]]
	...
	--[[
		elseif starts_with(entity.name,"hero-turret-4") == true then
			...
			entity.kills = math.max(levelup_4, kills)
			entity.damage_dealt = math.max(levelup_damage_4, dmg)
		elseif starts_with(entity.name,"hero-turret-3") == true then
			...
			entity.kills = math.max(levelup_3, kills)
			entity.damage_dealt = math.max(levelup_damage_3, dgm)
		elseif starts_with(entity.name,"hero-turret-2") == true then		
			local dmg = entity.damage_dealt
			if dgm == nil then dgm = 0 end
			local kills = entity.kills
			if kills == nil then kills = 0 end
			entity.kills = math.max(levelup_2, kills)
			entity.damage_dealt = math.max(levelup_damage_2, dgm)
		elseif starts_with(entity.name,"hero-turret-1") == true then
			...
			entity.kills = math.max(levelup_1, kills)
			entity.damage_dealt = math.max(levelup_damage_1, dgm)
		end
	]]
	end
