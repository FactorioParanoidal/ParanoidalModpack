...
local function get_old_quantity(ingredients, old)
	...
	for i, ingredient in pairs(ingredients) do
		...
		if item then
			...
			error("recipe " .. recipe .. " contains an invalid ingredient")
		end
	end
	return amount
end
	...
	for i, ingredient in ipairs(ingredients) do
		...
		if item then -- duplicate value
			if items[item.name] then
				rebuild = true
				error("Duplicate item " .. item.name .. " found on recipe " .. recipe_name .. ".")
			else
			...
			rebuild = true
			error("Invalid item found on recipe " .. recipe_name .. ".")
		end
	end
...
function bobmods.lib.recipe.ingredients_cleanup()
	...
	log("recipe ingredients cleaned up...")
end
