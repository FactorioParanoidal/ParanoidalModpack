	...
	for i, prerequisite in ipairs(prerequisites) do
		if type(prerequisite) == "string" then
			if prerequisites_list[prerequisite] then -- duplicate value
				error("in " .. technology_name .. " duplicate prerequisite " .. prerequisite)
				rebuild = true
			else
				prerequisites_list[prerequisite] = true
			end
		else --invalid value
			error("in " .. technology_name .. " invalid prerequisite " .. prerequisite)
			rebuild = true
		end
	end
...
local function duplicate_prerequisites_check(technology_name)
	if type(technology_name) == "string" and data.raw.technology[technology_name] then
		...
		if
			data.raw.technology[technology_name].normal and data.raw.technology[technology_name].normal.prerequisites
		then
		...
		if
			data.raw.technology[technology_name].expensive
			and data.raw.technology[technology_name].expensive.prerequisites
		then
			local prerequisites = duplicate_prerequisites_check_inner(
				technology_name,
...
function bobmods.lib.tech.prerequisite_cleanup()
	...
	log("technology prerequisite cleaned up...")
end
