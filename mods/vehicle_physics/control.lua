...
function is_flycar(entity_name)
	storage.is_flycar[entity_name] = nil
	if storage.is_flycar[entity_name] == nil then
		if prototypes.entity[entity_name].collision_mask and prototypes.entity[entity_name].collision_mask.layers and prototypes.entity[entity_name].collision_mask.layers["player"] then
...
if event.entity and event.entity.type == "car" and not exclusions[event.entity.name] then
	...
	elseif not storage.cars[event.entity.unit_number] then
		...
		--	storage.cars[event.entity.unit_number] = {entity = event.entity,drift={x=0,y=0}, position = event.entity.position,idle_ticks = 0, orientation = event.entity.orientation, last_pos = event.entity.position}
		--	if event.entity.speed ~= 0 then
		--		storage.cars[event.entity.unit_number].drift = projection(event.entity.orientation, event.entity.speed)
		--	end
		--else
			if not storage.on_tick[event.tick+1] then
				storage.on_tick[event.tick+1] = {}
			end
			table.insert(storage.on_tick[event.tick+1], {
				func = function(vars)
					if vars.entity and vars.entity.valid then
						storage.cars[vars.entity.unit_number] = {entity = vars.entity,drift={x=0,y=0}, position = vars.entity.position,idle_ticks = 0, orientation = vars.entity.orientation, last_pos = vars.entity.position, last_speed = vars.entity.speed}
						if vars.entity.speed ~= 0 then
							storage.cars[vars.entity.unit_number].drift = projection(vars.entity.orientation, vars.entity.speed)
						end
						--game.print(storage.cars[vars.entity.unit_number].entity.name)
					end
				end,
...
function make_tire_marks(surface,position,speed,unit_number)
	...
	--game.print(tilename)
	--if tilename:find("sand") or tilename:find("desert") or tilename:find("dirt") or tilename:find("grass") then
	--	return
	--end
	if tile.prototype.vehicle_friction_modifier >0.9 then
		return
	end
...
script.on_event(defines.events.on_tick, function(event)
	...
	for unit_number, tbl in pairs(storage.cars) do
		...
		if tbl.entity and tbl.entity.valid then
			...
			if speed == 0 and tbl.last_speed ~= 0 and not flycar then
			...
			--if speed ~=0 or surroundings == 1 then
					...
					if concrete_tiles >=1 then
						...
						if is_braking then
							...
							elseif orientation_difference < 0.029 then
								--better than vanilla braking on asphalt
								speed = speed -0.004
								make_no_tire_marks = true
								--drifting_multiplier = math.min(0.95,math.max(0,0.6+math.max(0,0.7-tbl.entity.speed)*0.5-tbl.idle_ticks*0.02+speed*0.45))*orientation_difference*10
								--game.print(orientation_difference)
				...
				if not is_braking then
					...
					if not flycar and concrete_tiles >=1 then
						local manuverability = 0.3 - math.abs(speed)/1.5
						--game.print("manuver: "..manuverability)
						local orientation_change = entity_orientation - tbl.orientation
						if orientation_change < -0.5 then
							orientation_change = orientation_change +1
						...
						entity_orientation = entity_orientation + orientation_change * (manuverability/4)
						tbl.entity.orientation = entity_orientation
					...
					elseif not flycar then
						...
						entity_orientation = entity_orientation + orientation_change * (manuverability/6)
						tbl.entity.orientation = entity_orientation
						if 	(storage.geigers[unit_number] or 0)+12  < event.tick then
							local volume = math.max(0.2, math.min(0.8, speed^0.4*1.2))*0.75
							--local rnd = math.ceil(math.random()*2.99999+0.000001)
							--local rnd = math.floor(math.random()*1.99)
							storage.geigers[unit_number] = event.tick
							
							tbl.entity.surface.play_sound({path = "vehphy-dirt", volume_modifier =volume, position = pos})
				...
				local temp = orientation_from_coords{x=drift_x,y=drift_y}-entity_orientation
				if temp > 0.7 then
					temp = temp-1
				elseif temp < -0.7 then
					temp = temp + 1
				end
				--if (1-math.max(0,math.abs(temp)-0.1)/5)<1 then
				--game.print(temp)
				--end
				speed = speed*(1-math.max(0,math.abs(temp)-0.1)/5)
				if (speed < original_speed-0.003) then
					tbl.entity.speed = original_speed-(original_speed-speed-0.003)
					--game.print(tbl.entity.speed-original_speed)
				end
				--game.print("2 "..(entity_orientation or "nil"))
				--game.print("3 "..(orientation_difference or "nil"))
			else
				tbl.drift = {x=0,y=0}
				tbl.last_pos = pos
			end
	...
	if storage.on_tick[event.tick] then
		for _, tbl in pairs(storage.on_tick[event.tick]) do
			tbl.func(tbl.vars)
		end
...
script.on_configuration_changed(function()
	...
	storage.is_flycar = {}
	for a,tbl in pairs(storage.cars) do
		if not tbl.entity or not tbl.entity.valid or exclusions[tbl.entity.name] then
			storage.cars[a] = nil
		end
	end
end)
