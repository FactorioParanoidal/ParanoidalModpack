...
function MergingChests.CreateEntity(data, name, loc_name, subgroup, width, height, sprite, connector)
	return
	{
		type = "container",
		name = name,
		...
		minable = { mining_time = 2, result = data.id, count = width * height },
		placeable_by = { item = data.id, count = width * height },
		max_health = 350,
		...
		inventory_size = MergingChests.LimitInventorySize(data.id, width * height),
		subgroup = subgroup,
		picture =
		{
			layers = sprite
