...
-- example: Tbl = {1, 2, 3, 4, 5, 6, 7}, k = 5
-- rolls: {5, 2, 3, 2, 2}
-- result: {5, 2, 3, 4, 6}
function mathUtils.getRandomElementIndexes(Tbl, k)
	local result = {}
	local tableAsArray = {}
	for index, value in pairs(Tbl) do
		tableAsArray[#tableAsArray+1] = {index = index}		--, value = value}
	end
	local tblSize = #tableAsArray
	local rndRolls = {}
	for i = 1, k do
		local roll = mRandom(tblSize)
		--game.print("roll "..roll.."/"..tblSize)	-- debug
		tblSize = tblSize - 1 
		while true do
			if not rndRolls[roll] then
				rndRolls[roll] = 1
				break
			else
				rndRolls[roll] = rndRolls[roll] + 1
				roll = roll + (rndRolls[roll] - 1)
				--game.print("roll -->"..roll)	-- debug
			end
		end
		result[#result+1] = tableAsArray[roll].index
	end
	return result
end
