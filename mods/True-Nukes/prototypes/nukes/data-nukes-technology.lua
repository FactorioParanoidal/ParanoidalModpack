...
if
	not settings.startup["keep-atomic-bomb-without-changes"]
	and not settings.startup["enable-compact-medium-atomics"].value
	and not settings.startup["enable-medium-atomics"].value
then
	data.raw.technology["atomic-bomb"].effects[1] = nil
end
data.raw.technology["atomic-bomb"].prerequisites =
	{ "basic-atomic-weapons", "rocket-control-unit", "rocket-fuel", "rocketry" }
...
if
	settings.startup["enable-medium-atomics"].value
	or settings.startup["enable-large-atomics"].value
	or settings.startup["enable-compact-15kt"].value
then
	data:extend({
		{
			type = "technology",
...
if
	settings.startup["enable-compact-medium-atomics"].value or settings.startup["enable-compact-small-atomics"].value
then
	data:extend({
		{
			type = "technology",
...
if settings.startup["enable-compact-15kt"].value or settings.startup["enable-compact-large-atomics"].value then
	...
	if
		settings.startup["enable-compact-medium-atomics"].value
		or settings.startup["enable-compact-small-atomics"].value
	then
		...
		if settings.startup["enable-nuclear-tests"].value and canDoTest then
			table.insert(
				data.raw.technology["compact-full-fission-weapons"].unit.ingredients,
				{ "test-pack-atomic-20t-3", 1 }
			)
		end
	elseif
		settings.startup["enable-compact-medium-atomics"].value
		or settings.startup["enable-compact-small-atomics"].value
		or settings.startup["enable-compact-large-atomics"].value
	then
		if nuke_materials.smallBoomMaterial == "californium" then
	...
	if
		data.raw.technology["compact-full-fission-weapons"]
		and data.raw.technology["compact-full-fission-weapons"].unit
		and #data.raw.technology["compact-full-fission-weapons"].unit.ingredients > 0
		and not next(data.raw.technology["compact-full-fission-weapons"].unit.ingredients[1])
	then
...
if
	settings.startup["enable-compact-small-atomics"].value
	or settings.startup["enable-compact-medium-atomics"].value
	or settings.startup["enable-compact-large-atomics"].value
	or settings.startup["enable-compact-15kt"].value
then
	data:extend({
