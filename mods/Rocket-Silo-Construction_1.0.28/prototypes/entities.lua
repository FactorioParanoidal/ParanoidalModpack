...
for s=1,6 do data:extend({ stages[s] }) end
...
if enable_se_cargo then
	...
	for s=1,6 do data:extend({ 
		update_proto_serlp(stages[s],'-serlp', cb,{"",{"entity-name.se-rocket-launch-pad"},{"RSC.construction_site",s}},icons_se_crs) 
		}) end
	end
end
...
if enable_se_probe then 
	...
	for s=1,6 do data:extend({ update_proto_serlp(stages[s],'-sesprs', cb,{"",{"entity-name.se-space-probe-rocket-silo"},{"RSC.construction_site",s}},icons_se_sp) }) end
